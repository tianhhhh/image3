...
predictor = dlib.shape_predictor(predictor_path)  #人脸关键点检测器
...
def landmark_dec_dlib_fun(img_src):
    ...
    for i in range(len(rects)):
        ...
        land_marks.append(land_marks_node)  #将关键点都加入目标矩阵中
    print(land_marks)
    return land_marks   #得到关键点
...
def face_thin_auto(src):
    ...
    for landmarks_node in landmarks: #图像中的关键点
...
def main():
    src = cv2.imread('D:/ComputerView/AIMakeup-master/raw/10.jpg')
    cv2.imshow('src', src)
    face_thin_auto(src)
    cv2.waitKey(0)
